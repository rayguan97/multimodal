...
class SunRGBDDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        text = [self.id_dict[str(i)] for i in self.img_anno[idx]]
...
def collator(batch):
    ...
    texts = [torch.LongTensor(x[1]["input_ids"]) for x in batch]
    return images, texts
...
def main():
    ...
    dataset = SunRGBDDataset(root_dir=args.data_root, anno_name=args.annotations, transform=transform2)
    ...
    text_tmp = torch.cat(all_text, dim=0)
    
    text_unique, idx, idx_rev = np.unique(text_tmp, return_index=True, return_inverse=True, axis=0)


    for t in tqdm(text_unique):
        _, text_emb = flava.encode_text(torch.tensor([t]).to(device), projection=True)
        text_embeds.append(text_emb.detach().cpu())
    ...
    structured_img_to_word = []
    for row in all_text:
        lst = []
        for encoding in row:
            lst.append(np.where((text_unique == np.array(encoding)).all(axis=1))[0][0])
            
        structured_img_to_word.append(lst)
    
    # text_unique, idx = np.unique(text_embeds, return_index=True, axis=0)
    
    anno_lst1 = defaultdict(list) # image id to words
    anno_lst2 = defaultdict(list) # word to image id
    
    for i, lst in enumerate(structured_img_to_word):
        anno_lst1[i].extend(lst)
    
    for i, lst in enumerate(structured_img_to_word):
        for id in lst:
            anno_lst2[id].append(i)
    ...
    image_to_text_relax_r1 = compute_recall_relax_multi(similarity_scores, anno_lst1, k=1)
    image_to_text_relax_r5 = compute_recall_relax_multi(similarity_scores, anno_lst1, k=5)
    text_to_image_relax_r1 = compute_recall_relax_multi(similarity_scores_t, anno_lst2, k=1)
    text_to_image_relax_r5 = compute_recall_relax_multi(similarity_scores_t, anno_lst2, k=5)
